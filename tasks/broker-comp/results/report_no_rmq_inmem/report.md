# RabbitMQ vs Redis Streams — Benchmark Report (excl. RabbitMQ in-memory / no-ack)
...
_`Avg loss (%)` is the mean of `lost_count / sent_count × 100` per row (`lost_count` = seq-gap estimate from the consumer)._
...
- **This document**: charts and summary use only rows where it is **not** (`broker` = `rmq` and `config` = `inmemory_noack`). Redis `inmemory_noack` and RabbitMQ `durable_ack` are still included.
...
*Generated automatically by `runner/report_no_rmq_inmem.py`*
